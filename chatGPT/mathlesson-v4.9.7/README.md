...
Current packaged build: `mathlesson.v4.9.7.html`.
...
```text
mathlesson-v4.9.7-current-package/
  README.md
  START_HERE.md
  mathlesson.v4.9.7.html
  OpeningManim-1.mp4
  SineCurveUnitCircle-1.mp4
  MovingAngle-1.mp4
  checklists/
    mathlesson-v4.9.7-master-test-checklist.md
  ...
  reference-lessons/
  roadmap/
    mathlesson-roadmap-v4.9.7-iphone-small-screen-hotfix.md
    earlier roadmap versions...
```


## v4.9.7 iPhone / small-screen stabilization

v4.9.7 is a mobile usability patch over v4.9.6. It targets iPhone portrait-mode blockers found during Netlify testing.

Small-screen changes include:

- native text/LaTeX inputs for mobile math assessment fields;
- compact MathLive behavior for remaining math fields;
- explicit mobile keyboard dismissal behavior;
- iOS viewport and safe-area CSS using dynamic viewport sizing;
- improved reachability for quiz controls on phones;
- improved UX Stats modal scrolling and close/export controls on phones;
- improved Lesson Editor scrolling and sticky action controls on phones;
- matching mobile input behavior in exported student lessons.

Use `checklists/mathlesson-v4.9.7-master-test-checklist.md` for the mobile validation pass.

## v4.9.6 inline exercise and answer-checking contract

MathLesson supports several answer-checking modes for authored assessment items. As of v4.9.6, bare inline exercise widgets with only `data-*` attributes are automatically expanded into visible input controls and a Check button:



Inline exercise widgets may be written either as a full HTML block with explicit controls or as a compact bare block such as:
...
When rendered, the app supplies the prompt, text/math input, Check button, result area, and hint area if they are missing.
...
3. Open `mathlesson.v4.9.7.html` in a browser.
...
`checklists/mathlesson-v4.9.7-master-test-checklist.md`
...
This version keeps regex answer checking and hardens the answer-checking route. Text and regex answers are checked before symbolic parsing, and MathLive/ComputeEngine parsing is avoided for plain text answers. Use `reference-lessons/answer-checking-diagnostics-v1.1.json` to verify exact, alternate, regex, numeric, and symbolic checks.

### v4.9.6 inline exercise UI hotfix

This version fixes compact inline exercise blocks that previously rendered as empty red bars. Bare `<div class="exercise-widget" ...></div>` blocks now auto-expand into a visible prompt, input control, Check button, result area, and hint area. Use `reference-lessons/answer-checking-diagnostics-v1.1.json` to verify exact, alternate, regex, numeric, and symbolic checks.
